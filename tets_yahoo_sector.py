...
Usage:
  ...
  python sector_scanner.py --date 2026-06-24 --snapshot

  # Get snapshot for today (waits for 9:45 AM)
  python sector_scanner.py --date 2026-06-25 --snapshot

  # Relax filters
  python sector_scanner.py --date 2026-06-24 --snapshot --no-rvol --no-etf
"""
...
SECTOR_TICKERS: Dict[str, List[str]] = {
    "Technology": [
        ...
        "MU", "NOW", "AMAT", "KLAC", "LRCX", "ADI", "MRVL", "PANW", "SNPS", "CDNS","MSTR","APP"
    ],
    "Healthcare": [
    ...
    "Financials": [
        "JPM", "V", "MA", "BAC", "WFC", "GS", "MS", "AXP", "BLK","COIN",
...
def fetch_historical_snapshot(symbols: List[str], target_date: date, target_time: str = "09:45") -> Dict[str, Dict]:
    """
    ...
    log.info(f"📊 Fetching historical data for {len(symbols)} symbols at {target_time} EST...")
        ...
        for sym in chunk:
            try:
                ...
                day_data['time_diff'] = abs(
                    day_data.index.time - datetime.min.time().replace(hour=t_hour, minute=t_min)
                )
                closest_idx = day_data['time_diff'].idxmin()
                row = day_data.loc[closest_idx]

                # Get previous day close
                prev_day = hist_est[hist_est.index.strftime("%Y-%m-%d") == prev_date.strftime("%Y-%m-%d")]
                prev_close = float(prev_day['Close'].iloc[-1]) if not prev_day.empty else 0
...
def avg_daily_volume_20d(symbol: str, target_date: Optional[date] = None) -> Optional[float]:
...
def vwap_from_bars(symbol: str, target_date: Optional[date] = None, target_time: str = "09:45") -> Optional[float]:
...
def scan_day_snapshot(scan_date: date, snapshot_time: str = "09:45", wait: bool = True) -> Dict:
    ...
    if is_past:
        ...
        log.info(f"📸 Capturing LIVE snapshot at: {snapshot_dt.strftime('%H:%M:%S')} EST")
    ...
    for sym in ALL_TICKERS:
        ...
        if USE_RVOL:
            ...
            if avg_vol and avg_vol > 0:
                ...
                rvol = pm_vol / (avg_vol * 0.20)  # 20% of daily volume expected in premarket
...
def print_results(res: Dict):
    ...
    print(f"\n{'═' * 80}")
    print(f"  📸 SNAPSHOT SCANNER - {'HISTORICAL' if res.get('is_historical') else 'LIVE'}")
...
def save_snapshot(res: Dict, out: str = "."):
    ...
    for sector, df in res["top_sectors"].items():
        p = os.path.join(date_folder, f"snapshot_{timestamp}_{sector.replace(' ', '_')}.csv")
...
def parse_args():
    p = argparse.ArgumentParser(description="S&P 500 Premarket Scanner - Historical Snapshots")
...
def main():
    ...
    if scan_date < today and not args.snapshot:
        ...
        log.info(f"   Running: python sector_scanner.py --date {args.date} --snapshot")
        return
