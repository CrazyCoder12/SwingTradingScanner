...
Delay note:
  Free IEX feed = 15-min delayed.
  • At 9:25 AM EST you see data up to ~9:10 AM only.
  • Best run time: 9:35–9:40 AM EST (full premarket covered, RTH bars available)
  • For real-time premarket: set ALPACA_DATA_FEED=sip (paid ~$9/mo)
...
Usage:
  python sector_scanner.py --date 2026-06-24
  python sector_scanner.py --start 2026-06-16 --end 2026-06-20
  python sector_scanner.py --date 2026-06-24 --no-rvol
  python sector_scanner.py --date 2026-06-24 --no-etf
  python sector_scanner.py --date 2026-06-24 --debug --csv
...
def fetch_bars_batch(symbols: list[str], start: datetime, end: datetime,
...
#
#  Works on free IEX feed. No SIP required.
    ...
    for sector, etf_gap_pct in top_etf_sectors:
        ...
        for sym in tickers:
            ...
            rows.append({
                ...
                "open_gain_pct": round(open_gain, 2)   if open_gain is not None else None,
            })
...
def scan_day(scan_date: date) -> dict:
    log.info("=" * 64)
    log.info("Scanning %s  |  feed=%s  |  top_sectors=%d  |  rvol=%s  |  etf_filter=%s",
             scan_date, DATA_FEED, TOP_SECTORS,
    ...
    log.info("Step 1/3  Fetching snapshots for %d symbols (stocks + ETFs)…",
             len(all_snap_syms))
    ...
    log.info("Step 2/3  Fetching PM 1-min bars for %d tickers with PM volume…",
             len(pm_candidates))
    pm_bars = pd.DataFrame()
    if pm_candidates:
        ...
        pm_count = (pm_bars.index.get_level_values("symbol").nunique()
                    if not pm_bars.empty and isinstance(pm_bars.index, pd.MultiIndex) else 0)
    ...
    rth_count = (rth_bars.index.get_level_values("symbol").nunique()
                 if not rth_bars.empty and isinstance(rth_bars.index, pd.MultiIndex) else 0)
    ...
    results  = []
    skip     = {"no_snap":0,"no_rth":0,"gap":0,"pmvol":0,"vwap":0,"rvol":0,"etf":0}
    ...
    for sym in ALL_TICKERS:
        ...
        if gap_pct < PREMARKET_GAP_MIN:
            skip["gap"] += 1
            if DEBUG: log.debug("SKIP %-8s  gap %.2f%%", sym, gap_pct*100)
            continue

        if pm_vol < PREMARKET_VOL_MIN:
            skip["pmvol"] += 1
            if DEBUG: log.debug("SKIP %-8s  pm_vol %,.0f", sym, pm_vol)
            continue

        if pm_vwap_val and not above_vwap:
            skip["vwap"] += 1
            if DEBUG: log.debug("SKIP %-8s  below VWAP %.2f (pm_close %.2f)", sym, pm_vwap_val, pm_close)
            continue
        ...
        if USE_ETF_FILTER and etf_gap < 0:
            skip["etf"] += 1
            if DEBUG: log.debug("SKIP %-8s  sector ETF gap %.2f%%", sym, etf_gap)
            continue
        ...
        if USE_RVOL:
            ...
            if rvol is not None and rvol < RVOL_MIN:
                skip["rvol"] += 1
                if DEBUG: log.debug("SKIP %-8s  rvol %.2fx", sym, rvol)
                continue
    ...
    log.info(
        ...
        len(results), skip["no_snap"], skip["no_rth"],
        skip["gap"], skip["pmvol"], skip["vwap"], skip["rvol"], skip["etf"],
    )
...
STOCK_COLS = [
    ...
    "rth_open","rth_high","rth_close","open_gain_pct",
]
STOCK_HDRS = [
    "Symbol","Prev $","PM Open","PM Last","PM VWAP",
    "Gap%","PM Rng%","PM Vol","RVol",
    "Score","Quality",
    "RTH Open","RTH High","RTH Close","Gain%",
]
...
def print_results(res: dict):
    ...
    print(flt)
    print(f"  NOTE   : Score=confluence 0–100. STRONG≥65, MODERATE≥40, WEAK<40 or below VWAP")
    print(f"{'═'*80}\n")
    ...
    for rank, (sector, df) in enumerate(res["top_sectors"].items(), 1):
        ...
        for _, r in df.iterrows():
            ...
            if r["pm_range_pct"] > 8:
                _bullets.append(f"extended PM range {r['pm_range_pct']:.1f}% → fade risk")
    ...
    if etf_stocks:
        print(f"\n\n{'═'*80}")
        print(f"  LIST B — ETF SECTOR MOMENTUM PLAYS  (top {TOP_ETF_SECTORS} ETF sectors)")
        print(f"  Logic : stocks LEADING their sector ETF in PM (ranked by RS vs ETF)")
        print(f"  Use   : sector rotation / broad-market options  (relaxed filters: gap≥0.5%, vol≥50K)")
        print(f"  RS vs ETF = stock gap% − sector ETF gap%  →  positive = outperforming the sector")
        print(f"{'═'*80}")

        ETF_COLS = [
            "symbol","prev_close","pm_open","pm_close","pm_vwap",
            "gap_pct","etf_gap_pct","rs_vs_etf","pm_range_pct","pm_volume",
            "rth_open","rth_close","open_gain_pct",
        ]
        ETF_HDRS = [
            "Symbol","Prev $","PM Open","PM Last","PM VWAP",
            "Stock Gap%","ETF Gap%","RS vs ETF","PM Rng%","PM Vol",
            "RTH Open","RTH Close","Gain%",
        ]
        ...
        for rank, (sector, df) in enumerate(etf_stocks.items(), 1):
            ...
            print(f"       {n_lead}/{len(df)} stocks outperforming the ETF (positive RS)")
            print(f"{'─'*80}")
            rows = df[ETF_COLS].values.tolist()
            print(tabulate(rows, headers=ETF_HDRS, floatfmt=".2f", tablefmt="rounded_outline"))
...
# =================================================================
# Strategy:
#   1. Use Alpaca Snapshot API  →  gets premarket OHLCV for ALL tickers in 1 call (no SIP needed)
#   2. Use prior-day daily bar  →  reliable prior close (no timing games)
#   3. Fetch 1-min RTH bars 9:30–9:35 for the open-window metrics
#
# Filters (all must pass):
#   ✓  Premarket gap  >= 2%    (pm_close vs prior_close)
#   ✓  Premarket vol  >= 100K
#   ✓  PM last price  >= PM VWAP
#   ✓  Relative vol   >= 2x    [optional — --no-rvol to skip]
#
# Usage:
#   python sector_scanner.py --date 2026-06-24
#   python sector_scanner.py --start 2026-06-16 --end 2026-06-20
#   python sector_scanner.py --date 2026-06-24 --no-rvol
#   python sector_scanner.py --date 2026-06-24 --debug
#   python sector_scanner.py --date 2026-06-24 --csv
#
# Env vars:
#   ALPACA_API_KEY
#   ALPACA_SECRET_KEY
...
# SECRET_KEY = os.getenv("ALPACA_SECRET_KEY", "YOUR_SECRET")
#
# # Snapshot + historical bars work on free IEX feed
# DATA_FEED = os.getenv("ALPACA_DATA_FEED", "iex")
#
# PREMARKET_GAP_MIN   = 0.02      # 2% minimum gap vs prior close
# PREMARKET_VOL_MIN   = 100_000   # minimum premarket shares traded
# USE_RVOL            = True
# RVOL_MIN            = 2.0
# TOP_SECTORS         = 2
# TOP_STOCKS_PER_SECT = 10
...
#     return prev
#
# def get_sym(df: pd.DataFrame, symbol: str) -> pd.DataFrame:
#     if df.empty:
#         return pd.DataFrame()
#     if isinstance(df.index, pd.MultiIndex):
#         try:
...
#
# def fetch_bars_batch(symbols: list[str], start: datetime, end: datetime,
#                      timeframe: TimeFrame, feed: str = None) -> pd.DataFrame:
...
#
#
# # ─── SNAPSHOT → PREMARKET DATA ───────────────────────────────────────────────
# #
# #  Alpaca's /v2/stocks/snapshots endpoint returns for each ticker:
# #    • latestTrade / latestQuote
# #    • minuteBar  (latest 1-min bar)
# #    • dailyBar   (current day)
# #    • prevDailyBar (previous trading day)  ← reliable prior close!
# #
# #  The "dailyBar" during pre-market covers 4:00–current time and includes
# #  premarket volume and OHLC.  This works on the FREE IEX feed.
# #
# #  We call the raw REST API directly because alpaca-py's SnapshotRequest
# #  does not expose the prevDailyBar fields in all SDK versions.
#
# def fetch_snapshots(symbols: list[str]) -> dict:
#     """
#     Returns dict: symbol → {
#         prev_close, pm_open, pm_high, pm_low, pm_close, pm_volume,
#         day_open, day_close, day_volume
#     }
#     Uses Alpaca REST v2 directly for full snapshot including prevDailyBar.
#     """
#     BASE = "https://data.alpaca.markets/v2/stocks/snapshots"
#     headers = {
...
#             data = resp.json()
#         except Exception as exc:
#             log.warning("snapshot fetch error chunk %d: %s", i // CHUNK, exc)
#             continue
#
#         for sym, snap in data.items():
#             try:
#                 prev  = snap.get("prevDailyBar") or {}
#                 day   = snap.get("dailyBar") or {}      # includes premarket on scan day
#                 # prevDailyBar.c  = prior day close
#                 prev_close = prev.get("c") or prev.get("cl")
#                 if prev_close is None:
#                     continue
#
#                 # dailyBar during premarket = premarket session data
#                 pm_open   = day.get("o") or day.get("op")
#                 pm_high   = day.get("h") or day.get("hi")
#                 pm_low    = day.get("l") or day.get("lo")
#                 pm_close  = day.get("c") or day.get("cl")
...
#                     "pm_low":     float(pm_low)    if pm_low    else None,
#                     "pm_close":   float(pm_close)  if pm_close  else None,
#                     "pm_volume":  float(pm_volume) if pm_volume else 0.0,
#                 }
#             except Exception as exc:
#                 log.debug("snapshot parse error %s: %s", sym, exc)
#
#     return result
#
#
# def fetch_premarket_bars(symbols: list[str], scan_date: date) -> pd.DataFrame:
#     """
#     1-min bars for the premarket window (4:00–9:29 EST).
#     Used to compute accurate premarket VWAP from the bar series.
#     Falls back to snapshot OHLCV-derived VWAP if bars are sparse.
#     """
#     start = to_utc(scan_date, 4, 0)
#     end   = to_utc(scan_date, 9, 29)
#     return fetch_bars_batch(symbols, start, end, TimeFrame(1, TimeFrameUnit.Minute))
#
#
# def avg_daily_volume_20d(symbol: str, scan_date: date) -> Optional[float]:
#     """20-day average full-day volume for RVOL denominator."""
#     prev    = prev_trading_day(scan_date)
#     start_d = scan_date - timedelta(days=35)
#     try:
#         req = StockBarsRequest(
...
# def scan_day(scan_date: date) -> dict:
#     log.info("=" * 64)
#     log.info("Scanning %s  |  feed=%s  |  rvol=%s",
#              scan_date, DATA_FEED, f">={RVOL_MIN}x" if USE_RVOL else "off")
#
#     tf1m = TimeFrame(1, TimeFrameUnit.Minute)
#
#     # ── Step 1: Snapshots  (prev close + premarket OHLCV in one REST call) ──
#     log.info("Step 1/3  Fetching snapshots for all %d tickers…", len(ALL_TICKERS))
#     snaps = fetch_snapshots(ALL_TICKERS)
#     log.info("  Got snapshot data for %d tickers", len(snaps))
#     if not snaps:
#         log.error("No snapshot data returned — check API keys / network")
#         return {}
#
#     # ── Step 2: Premarket 1-min bars for VWAP (only tickers with pm_vol > 0) ──
#     pm_candidates = [s for s, v in snaps.items() if v["pm_volume"] > 0]
#     log.info("Step 2/3  Fetching premarket 1-min bars for %d tickers with PM volume…",
#              len(pm_candidates))
#     pm_bars = pd.DataFrame()
#     if pm_candidates:
#         pm_bars = fetch_premarket_bars(pm_candidates, scan_date)
#         pm_count = pm_bars.index.get_level_values("symbol").nunique() if (
#             not pm_bars.empty and isinstance(pm_bars.index, pd.MultiIndex)) else 0
#         log.info("  1-min bars returned for %d tickers", pm_count)
#
#     # ── Step 3: RTH open-window bars 9:30–9:35 ──
#     log.info("Step 3/3  Fetching 9:30–9:35 AM RTH bars…")
#     rth_bars = fetch_bars_batch(
#         ALL_TICKERS,
#         to_utc(scan_date, 9, 30),
#         to_utc(scan_date, 9, 36),
#         tf1m,
#     )
#     rth_count = rth_bars.index.get_level_values("symbol").nunique() if (
#         not rth_bars.empty and isinstance(rth_bars.index, pd.MultiIndex)) else 0
#     log.info("  RTH bars for %d tickers", rth_count)
#
#     if rth_bars.empty:
#         log.warning("No RTH bars — market closed on %s?", scan_date)
...
#     # ── Per-ticker processing ──────────────────────────────────────────────────
#     results = []
#     skip = {"no_snap": 0, "no_rth": 0, "gap": 0, "pmvol": 0, "vwap": 0, "rvol": 0}
#     rvol_cache: dict[str, Optional[float]] = {}
#
#     for sym in ALL_TICKERS:
#         snap = snaps.get(sym)
...
#         pc        = snap["prev_close"]
#         pm_vol    = snap["pm_volume"]
#         pm_close  = snap["pm_close"]   # premarket last price
#         pm_open   = snap["pm_open"]
#         pm_high   = snap["pm_high"]
#         pm_low    = snap["pm_low"]
#
#         # ── VWAP: use 1-min bars if available, else snapshot OHLCV ──
#         sym_pm_bars = get_sym(pm_bars, sym)
#         if not sym_pm_bars.empty:
#             pm_vwap_val = vwap_from_bars(sym_pm_bars)
#         elif pm_close and pm_high and pm_low and pm_vol > 0:
#             # Approximate VWAP from snapshot OHLC
#             pm_vwap_val = (pm_high + pm_low + pm_close) / 3
#         else:
#             pm_vwap_val = None
#
#         # ── Gap % : PM close vs prior close ──
...
#             continue
#
#         if pm_vwap_val and pm_close and pm_close < pm_vwap_val:
#             skip["vwap"] += 1
#             if DEBUG:
#                 log.debug("  SKIP %-8s  pm_close %.2f < vwap %.2f", sym, pm_close, pm_vwap_val)
#             continue
#
#         # RVOL (optional)
#         rvol = None
...
#             avg_vol = rvol_cache[sym]
#             if avg_vol and avg_vol > 0:
#                 # Compare PM volume to expected PM volume (~20% of daily avg)
...
#             "prev_close":    round(pc, 2),
#             "pm_open":       round(pm_open, 2)     if pm_open      else None,
#             "pm_close":      round(pm_close, 2)    if pm_close     else None,
#             "pm_vwap":       round(pm_vwap_val, 2) if pm_vwap_val  else None,
#             "gap_pct":       round(gap_pct * 100, 2),
#             "pm_volume":     int(pm_vol),
#             "rvol":          round(rvol, 2)         if rvol         else "N/A",
...
#
#     log.info(
#         "  Results → pass=%d  |  no_snap=%d  no_rth=%d  gap=%d  pmvol=%d  vwap=%d  rvol=%d",
#         len(results), skip["no_snap"], skip["no_rth"],
#         skip["gap"], skip["pmvol"], skip["vwap"], skip["rvol"],
#     )
#
#     if not results:
#         log.warning(
#             "\n  No stocks passed all filters.\n"
#             "  Tip: run with --debug to see per-ticker rejection reasons.\n"
#             "  Also try:  --no-rvol  to disable the RVOL filter."
#         )
#         return {}
#
#     df = pd.DataFrame(results)
#
#     # Sector ranking: median open-gain of qualifying stocks
#     sector_perf = (
#         df.groupby("sector")["open_gain_pct"]
#         .agg(["median", "count"])
#         .rename(columns={"median": "median_gain_pct", "count": "qualifying_stocks"})
#         .sort_values("median_gain_pct", ascending=False)
#     )
#
#     top_sector_names = sector_perf.head(TOP_SECTORS).index.tolist()
#     log.info("Top %d sectors: %s", TOP_SECTORS, top_sector_names)
...
# # ─── OUTPUT ──────────────────────────────────────────────────────────────────
#
# COLS = ["symbol","prev_close","pm_open","pm_close","pm_vwap",
#         "gap_pct","pm_volume","rvol","rth_open","rth_high","rth_close","open_gain_pct"]
# HDRS = ["Symbol","Prev Close","PM Open","PM Last","PM VWAP",
#         "Gap %","PM Vol","RVol","RTH Open","RTH High","RTH Close","Open Gain %"]
#
# def print_results(res: dict):
#     if not res:
#         return
#     print(f"\n{'═'*76}")
#     print(f"  DATE   : {res['date']}")
#     print(f"  WINDOW : 9:30–9:35 AM EST  |  FEED: {DATA_FEED.upper()}")
#     flt = (f"  FILTERS: Gap≥{PREMARKET_GAP_MIN*100:.0f}%  PM_Vol≥{PREMARKET_VOL_MIN:,}"
#            f"  Above_PM_VWAP")
#     flt += f"  RVOL≥{RVOL_MIN}x" if USE_RVOL else "  (RVOL off)"
#     print(flt)
...
#
#     for rank, (sector, df) in enumerate(res["top_sectors"].items(), 1):
#         med = res["sector_ranks"].loc[sector, "median_gain_pct"]
#         cnt = int(res["sector_ranks"].loc[sector, "qualifying_stocks"])
#         print(f"\n{'─'*76}")
#         print(f"  #{rank}  {sector.upper()}")
#         print(f"       Median open gain: {med:+.2f}%  |  Qualifying stocks: {cnt}")
#         print(f"{'─'*76}")
#         print(tabulate(df[COLS].values.tolist(), headers=HDRS,
...
#     p.add_argument("--feed",  default=None, help="iex or sip")
#     p.add_argument("--debug", action="store_true")
#     p.set_defaults(use_rvol=True)
#     return p.parse_args()
#
# def main():
#     global USE_RVOL, DATA_FEED, DEBUG
#     args     = parse_args()
#     USE_RVOL = args.use_rvol
#     DEBUG    = args.debug
#     if args.feed:
...
#
#     if not all_results:
#         print("\nNo results. Try --no-rvol or --debug to diagnose.")
#     else:
#         print(f"\n✓  Scanned {len(all_results)} trading day(s).")
#
# if __name__ == "__main__":
