...
def fetch_bars(symbol: str, timeframe: str,
...
def get_signals_s1(df: pd.DataFrame, start_dt: datetime) -> list:
    ...
    for i in range(max(1, start_pos), len(df)):
        ...
        if bull and close > e5:
            signals.append({"signal_date": df.index[i].strftime("%Y-%m-%d"),
                            "direction": "BULLISH", "scanner": "S1"})
        elif bear and close < e5:
            signals.append({"signal_date": df.index[i].strftime("%Y-%m-%d"),
                            "direction": "BEARISH", "scanner": "S1"})
    return signals
...
def get_signals_s2(df: pd.DataFrame, start_dt: datetime) -> list:
    ...
    for i in range(max(1, start_pos), len(df)):
        ...
        if bull and e5 > e50:
            signals.append({"signal_date": df.index[i].strftime("%Y-%m-%d"),
                            "direction": "BULLISH", "scanner": "S2"})
        elif bear and e5 < e50:
            signals.append({"signal_date": df.index[i].strftime("%Y-%m-%d"),
                            "direction": "BEARISH", "scanner": "S2"})
    return signals


def run_daily_scanner(symbols: list, start_date_str: str,
                      run_s1: bool = True, run_s2: bool = True) -> pd.DataFrame:
    ...
    for i, sym in enumerate(symbols, 1):
        ...
        df = fetch_bars(sym, "1Day",
                        warmup_dt.replace(tzinfo=timezone.utc), now_utc)
...
def get_intraday(symbol: str, timeframe: str,
                 fetch_start: datetime, fetch_end: datetime) -> pd.DataFrame:
...
def bars_for_date(df: pd.DataFrame, target_date: datelib,
                  after_time: str = None, before_time: str = None) -> pd.DataFrame:
...
def premarket_ema_state(bars_10m: pd.DataFrame,
                        entry_date: datelib, direction: str) -> str:
    pm_bars = bars_for_date(bars_10m, entry_date,
                             after_time=PREMARKET_START, before_time=MARKET_OPEN)
    if pm_bars.empty:
        return "UNKNOWN"
    last = pm_bars.iloc[-1]
...
def find_entry(bars_10m: pd.DataFrame, entry_date: datelib,
               direction: str, pm_state: str):
    """
...
class RLevels:
    ...
    """
    def __init__(self, entry_price: float, ema12_at_entry: float,
                 direction: str):
    ...
    def __repr__(self):
        ...
        return (f"RLevels({self.direction}  entry={self.entry:.3f}  "
...
def _price_hit(bar_high: float, bar_low: float,
...
def simulate_tiered_exits(bars: pd.DataFrame, entry_ts,
                           rl: RLevels,
                           use_ema_stop: bool,
                           stop_timeframe_bars: pd.DataFrame | None = None) -> dict:
    """
    ...
    for ts, row in future.iterrows():
        ...
        if hit_2r and not hit_3r and shares_remaining > 0 \
            ...
            total_pnl       += pnl
            shares_remaining -= sell_shares
            fills.append({"ts": ts, "price": rl.target_3r,
                          "shares": round(sell_shares, 6),
                          "reason": "TARGET_3R", "pnl": round(pnl, 2)})
            hit_3r       = True
            trail_active = True
        ...
        if trail_active and shares_remaining > 0 and trail_stop is not None:
            ...
            if trail_hit:
                ...
                total_pnl       += pnl
                fills.append({"ts": ts, "price": round(exit_p, 4),
                              "shares": round(shares_remaining, 6),
                              "reason": "TRAIL_STOP", "pnl": round(pnl, 2)})
                shares_remaining = 0
                break
        ...
        if stop_hit or manual_stop_hit:
            ...
            reason = "STOP_BE" if (hit_2r and manual_stop_hit) else \
                     ("STOP_10M" if use_ema_stop else "STOP_1H")
            pnl = rl.pnl_dollar(exit_p, shares_remaining)
            total_pnl       += pnl
            fills.append({"ts": ts, "price": round(exit_p, 4),
                          "shares": round(shares_remaining, 6),
    ...
    if shares_remaining > 0 and not future.empty:
        ...
        pnl = rl.pnl_dollar(exit_p, shares_remaining)
        total_pnl += pnl
        fills.append({"ts": future.index[-1], "price": round(exit_p, 4),
                      "shares": round(shares_remaining, 6),
                      "reason": "MAX_HOLD", "pnl": round(pnl, 2)})
        shares_remaining = 0
...
def run_trade_s1(symbol: str, signal_date: str, direction: str) -> dict | None:
    ...
    fetch_start = datetime(entry_date.year, entry_date.month, entry_date.day,
                           3, 0, tzinfo=ET).astimezone(timezone.utc)
    fetch_end   = min(
        datetime(entry_date.year, entry_date.month, entry_date.day,
                 tzinfo=ET) + timedelta(days=MAX_HOLD_DAYS + 3),
        today_utc
    )
    ...
    result = simulate_tiered_exits(bars_10m, entry_ts, rl,
                                   use_ema_stop=True)
    if result is None:
        return None
...
def print_fills(trade: dict):
    ...
    print(f"      Fills:")
    for f in fills:
        ts_str = f["ts"].strftime("%Y-%m-%d %H:%M") if hasattr(f["ts"], "strftime") else str(f["ts"])
        pnl_sign = "+" if f["pnl"] >= 0 else ""
        print(f"        {ts_str}  {f['reason']:<12}  "
              f"{f['shares']:.4f} sh @ ${f['price']:.3f}  "
              f"P&L: {pnl_sign}${f['pnl']:.2f}")


def print_report(trades: list, df: pd.DataFrame, title: str, verbose_fills: bool = False):
    if df.empty:
    ...
    print(f"  {title}")
    print(f"{'='*W}")
    print(f"  Trades        : {len(df)}  (🟢 {len(wins)} wins  🔴 {len(losses)} losses)")
    print(f"  Win rate      : {win_rate:.1f}%")
    print(f"  Avg win       : +{avg_win:.2f}%       Avg loss: {avg_loss:.2f}%")
    print(f"  Best trade    : {best['symbol']} {best['pnl_pct']:+.2f}%  (${best['pnl_dollar']:+.2f})")
    print(f"  Worst trade   : {worst['symbol']} {worst['pnl_pct']:+.2f}%  (${worst['pnl_dollar']:+.2f})")
    print(f"  Total P&L     : ${total_pnl:+,.2f}")
    print(f"  Portfolio     : ${final_port:,.2f}  ({port_ret:+.2f}% return on ${STARTING_CAPITAL:,.0f})")
    print(f"\n  Position sizing (avg per trade):")
    print(f"    Risk (1R $)   : ${RISK_PER_TRADE:.0f}  |  Avg 1R distance: ${avg_risk:.3f}")
    print(f"    Avg shares    : {avg_shares:.2f}  |  Avg capital deployed: ${avg_cap:.2f}")
    print(f"    Trades hitting 2R: {hit_2r_pct:.1f}%    Trades hitting 3R: {hit_3r_pct:.1f}%")
    ...
    if "pm_state" in df.columns:
        ...
        for state, grp in df.groupby("pm_state"):
            ...
            print(f"    {state:<10}  {len(grp):>3} trade(s)  win rate {wr:.0f}%  avg {avg:+.2f}%")
    ...
    for idx, (_, r) in enumerate(df.iterrows()):
        ...
        print(f"  {int(r['trade_#']):<4} {r['symbol']:<7} {r['direction']:<8} {pm:<8} "
        ...
        if verbose_fills and idx < len(trades):
            ...
            match = next((t for t in trades
                          if t.get("symbol") == r["symbol"]
                          and t.get("entry_date") == r["entry_date"]), None)
            if match:
                print_fills(match)
...
def main():
    ...
    parser.add_argument("--strategy",     choices=["1", "2", "both"], default="both",
                        help="Backtest strategy: 1=10m stop, 2=1h stop (default: both)")
    parser.add_argument("--no-bearish",   action="store_true",
                        help="Skip bearish signals (long-only)")
    parser.add_argument("--fills",        action="store_true",
                        help="Print detailed fill breakdown per trade")
    parser.add_argument("--out",          default=None,
                        help="Output CSV prefix (default: bt_alpaca_YYYYMMDD)")
    ...
    for _, row in sig_df.iterrows():
        ...
        if args.strategy in ("1", "both"):
            ...
            if t:
                ...
                parts.append(f"S1: {t['pnl_pct']:+.2f}%  ${t['pnl_dollar']:+.2f}  "
        ...
        if args.strategy in ("2", "both"):
            ...
            if t:
                ...
                h2 = "2R✓" if t["hit_2r"] else "2R✗"
                h3 = "3R✓" if t["hit_3r"] else "3R✗"
                parts.append(f"S2: {t['pnl_pct']:+.2f}%  ${t['pnl_dollar']:+.2f}  "
                             f"{h2} {h3}  [{t['exit_reason']}]")
            else:
...
# """
# EMA Backtest Engine — Alpaca API
# ==================================
# Extends swing_trading.py with two intraday backtest strategies.
#
# SIGNAL SOURCES (from swing_trading.py):
...
#       Option B: EMA-5 crosses BELOW EMA-12 AND 10m candle closes BELOW EMA-5
#
#   No entry signal by EOD → skip trade.
#   Position size: $1,000 per trade from $100,000 portfolio.
#
# STRATEGY 1 — 10m Stop Loss
#   Stop: 10m candle closes BELOW EMA-12  OR  EMA-5 crosses BELOW EMA-12 with close < EMA-5
#         (mirror for bearish)
#   Carry forward if EOD reached with no stop hit.
#
# STRATEGY 2 — 1h Stop Loss + 3 % Trailing
#   Stop: 1h EMA-5 crosses EMA-12 OR 1h close below EMA-12 (bearish: above)
#   Trail: once position is up ≥ 3 %, activate 1 % trailing stop from high watermark
#          (trail floor rises with each new 1h high for long, falls for short)
#
# Data source: Alpaca Markets v2 (APCA-API-KEY-ID / APCA-API-SECRET-KEY from .env)
#              Uses SIP feed (or IEX as fallback) for both daily and intraday bars.
#
# Usage:
#   python3 backtest_alpaca.py --start-date 2026-03-01
#   python3 backtest_alpaca.py --start-date 2026-03-01 --scanner 1
...
# TRAILING_TRIGGER    = 0.03           # 3 % gain to activate trailing stop
# TRAILING_STOP_FROM_HIGH = 0.01       # 1 % below high watermark for longs
# MAX_HOLD_DAYS       = 20             # bail-out after this many calendar days
# WARMUP_DAYS         = 120            # daily bars before start_date for EMA warmup
# ET = ZoneInfo("America/New_York")
#
# MARKET_OPEN  = "09:30"
...
#     Generic Alpaca v2 bar fetcher.
#     timeframe: '1Day', '10Min', '1Hour'
#     Returns DataFrame indexed by UTC-aware datetime, columns: open high low close volume
#     """
#     url    = f"{_BASE}/v2/stocks/{symbol}/bars"
#     params = {
#         "timeframe":  timeframe,
...
# def run_daily_scanner(symbols: list, start_date_str: str,
#                       run_s1: bool = True, run_s2: bool = True) -> pd.DataFrame:
#     """Scan daily bars for all symbols and return combined signal DataFrame."""
#     start_dt  = datetime.strptime(start_date_str, "%Y-%m-%d")
#     warmup_dt = start_dt - timedelta(days=WARMUP_DAYS)
#     now_utc   = datetime.now(timezone.utc)
#
...
#
#         df = fetch_bars(sym, "1Day",
#                         warmup_dt.replace(tzinfo=timezone.utc),
#                         now_utc)
#         if df.empty or len(df) < 60:
#             print("no data")
#             continue
#
#         # Daily bars from Alpaca are UTC midnight; strip tz for date comparisons
#         df.index = df.index.tz_localize(None)
#         df = add_emas_daily(df)
#
#         parts = []
...
#
#     df_sig = pd.DataFrame(all_signals)
#     df_sig = df_sig.sort_values(["signal_date", "symbol"]).reset_index(drop=True)
#     print(f"\n  {len(df_sig)} total signal(s) across {df_sig['symbol'].nunique()} symbols.\n")
#     return df_sig
#
#
# # ─────────────────────────────────────────────────────────────────────────────
# #  Intraday data cache  (avoid re-fetching same symbol/day combo)
# # ─────────────────────────────────────────────────────────────────────────────
#
# _cache: dict = {}
#
...
#         df = fetch_bars(symbol, timeframe, fetch_start, fetch_end)
#         if not df.empty:
#             # Convert to ET for readable time comparisons
#             df.index = df.index.tz_convert(ET)
#         _cache[key] = df
#     return _cache[key]
#
...
# def bars_for_date(df: pd.DataFrame, target_date: datelib,
#                   after_time: str = None, before_time: str = None) -> pd.DataFrame:
#     """Filter intraday DataFrame to a specific calendar date (ET), with optional time window."""
#     mask = df.index.date == target_date
#     sub  = df[mask]
#     if sub.empty:
#         return sub
#     if after_time:
#         h, m = map(int, after_time.split(":"))
#         sub = sub[sub.index.time >= __import__("datetime").time(h, m)]
#     if before_time:
#         h, m = map(int, before_time.split(":"))
#         sub = sub[sub.index.time < __import__("datetime").time(h, m)]
#     return sub
...
#                         entry_date: datelib, direction: str) -> str:
#     """
#     Look at the last 10m bar between 4:00 and 9:29 ET.
#     Returns 'ALIGNED', 'AGAINST', or 'UNKNOWN'.
#     ALIGNED = pre-market EMAs support the intended direction.
#     """
#     pm_bars = bars_for_date(bars_10m, entry_date,
#                              after_time=PREMARKET_START, before_time=MARKET_OPEN)
#     if pm_bars.empty:
...
#                direction: str, pm_state: str):
#     """
#     Search for entry bar from 9:30 onward on entry_date.
#
#     If pre-market was ALIGNED:
#       → Enter on the 9:30 bar if EMA condition is satisfied immediately.
#     Then (regardless of pre-market):
#       → Look for first 10m bar where EMA-5 crosses EMA-12 in the right direction
#         AND close confirms (close > EMA-5 for bull, close < EMA-5 for bear).
#
#     Returns (entry_ts, entry_price) or (None, None).
#     """
#     mkt_bars = bars_for_date(bars_10m, entry_date, after_time=MARKET_OPEN)
#     if mkt_bars.empty:
#         return None, None
...
#
#         if direction == "BULLISH":
#             # Immediate entry at open if pre-market aligned AND condition holds
#             if i == 0 and pm_state == "ALIGNED" and e5 > e12:
#                 return ts, close
#             # Crossover entry
#             if prev_e5 is not None:
#                 cross = prev_e5 <= prev_e12 and e5 > e12
#                 if cross and close > e5:
#                     return ts, close
#             # Also accept: EMA already aligned and this is a continuation bar
#             # (EMA-5 was already above EMA-12 at open, still above)
#             if i == 0 and e5 > e12 and pm_state != "AGAINST":
#                 return ts, close
#
#         else:  # BEARISH
...
#
# # ─────────────────────────────────────────────────────────────────────────────
# #  Strategy 1 — 10m stop loss exit checker
# # ─────────────────────────────────────────────────────────────────────────────
#
# def check_exit_10m(bars_10m: pd.DataFrame, entry_ts,
#                    direction: str) -> dict:
#     """
#     Scan 10m bars AFTER entry_ts on the same and subsequent days.
#     Returns dict: {status, exit_ts, exit_price, reason}
#     status = 'EXIT' | 'CARRY'
#     """
#     subset = bars_10m[bars_10m.index > entry_ts]
#     if subset.empty:
#         return {"status": "CARRY"}
#
#     prev_e5  = float(bars_10m.loc[entry_ts, "ema5"])
#     prev_e12 = float(bars_10m.loc[entry_ts, "ema12"])
#
...
#
#         if direction == "BULLISH":
#             cross_bear = prev_e5 >= prev_e12 and e5 < e12
#             if close < e12 or (cross_bear and close < e5):
#                 return {"status": "EXIT", "exit_ts": ts,
#                         "exit_price": close, "reason": "STOP_10M"}
#         else:
#             cross_bull = prev_e5 <= prev_e12 and e5 > e12
#             if close > e12 or (cross_bull and close > e5):
#                 return {"status": "EXIT", "exit_ts": ts,
#                         "exit_price": close, "reason": "STOP_10M"}
#
#         prev_e5, prev_e12 = e5, e12
...
#
# # ─────────────────────────────────────────────────────────────────────────────
# #  Strategy 2 — 1h stop loss + trailing exit checker
# # ─────────────────────────────────────────────────────────────────────────────
#
# def check_exit_1h(bars_1h: pd.DataFrame, from_ts,
#                   entry_price: float, direction: str,
#                   trail_active: bool, trail_stop: float) -> dict:
#     """
#     Scan 1h bars after from_ts.
#     Stop: 1h EMA cross or close beyond EMA-12.
#     Trail: activates at 3 %, then trails 1 % from high watermark.
#     """
#     subset = bars_1h[bars_1h.index > from_ts]
#     if subset.empty:
#         return {"status": "CARRY", "trail_active": trail_active,
#                 "trail_stop": trail_stop}
#
#     pre = bars_1h[bars_1h.index <= from_ts]
#     prev_e5  = float(pre.iloc[-1]["ema5"])  if not pre.empty else None
#     prev_e12 = float(pre.iloc[-1]["ema12"]) if not pre.empty else None
#
#     for ts, row in subset.iterrows():
#         close = float(row["close"])
#         high  = float(row["high"])
...
#         e12   = float(row["ema12"])
#
#         # Activate trailing stop at 3 % gain
#         if not trail_active:
#             gain = (close - entry_price) / entry_price if direction == "BULLISH" \
#                    else (entry_price - close) / entry_price
#             if gain >= TRAILING_TRIGGER:
#                 trail_active = True
#                 trail_stop = (entry_price * (1 + TRAILING_STOP_FROM_HIGH)
#                               if direction == "BULLISH"
#                               else entry_price * (1 - TRAILING_STOP_FROM_HIGH))
#
#         # Move trail with high watermark
#         if trail_active:
#             if direction == "BULLISH":
#                 trail_stop = max(trail_stop, high * (1 - TRAILING_STOP_FROM_HIGH))
#             else:
#                 trail_stop = min(trail_stop, low * (1 + TRAILING_STOP_FROM_HIGH))
#
#         # Check trailing hit
#         if trail_active:
#             if direction == "BULLISH" and close < trail_stop:
#                 return {"status": "EXIT", "exit_ts": ts, "exit_price": close,
#                         "reason": "TRAIL_STOP", "trail_active": trail_active,
#                         "trail_stop": trail_stop}
#             if direction == "BEARISH" and close > trail_stop:
#                 return {"status": "EXIT", "exit_ts": ts, "exit_price": close,
#                         "reason": "TRAIL_STOP", "trail_active": trail_active,
#                         "trail_stop": trail_stop}
#
#         # Check EMA stop
#         if prev_e5 is not None and prev_e12 is not None:
#             if direction == "BULLISH":
#                 cross_bear = prev_e5 >= prev_e12 and e5 < e12
#                 if close < e12 or (cross_bear and close < e5):
#                     return {"status": "EXIT", "exit_ts": ts, "exit_price": close,
#                             "reason": "STOP_1H", "trail_active": trail_active,
#                             "trail_stop": trail_stop}
#             else:
#                 cross_bull = prev_e5 <= prev_e12 and e5 > e12
#                 if close > e12 or (cross_bull and close > e5):
...
#
# # ─────────────────────────────────────────────────────────────────────────────
# #  Strategy 1 — full trade lifecycle
# # ─────────────────────────────────────────────────────────────────────────────
#
# def run_trade_s1(symbol: str, signal_date: str, direction: str) -> dict | None:
#     entry_date = next_trading_day(signal_date)
...
#
#     pm_state = premarket_ema_state(bars_10m, entry_date, direction)
#     entry_ts, entry_price = find_entry(bars_10m, entry_date, direction, pm_state)
#     if entry_ts is None:
#         return None
#
#     result = check_exit_10m(bars_10m, entry_ts, direction)
#
#     if result["status"] == "EXIT":
#         exit_ts    = result["exit_ts"]
#         exit_price = result["exit_price"]
#     else:
#         # Max hold — use last available bar
#         future = bars_10m[bars_10m.index > entry_ts]
#         if future.empty:
#             return None
#         last       = future.iloc[-1]
#         exit_ts    = future.index[-1]
#         exit_price = float(last["close"])
#         result["reason"] = "MAX_HOLD"
#
...
#     return {
#         "symbol":        symbol,
#         "scanner":       None,           # filled by caller
#         "direction":     direction,
#         "signal_date":   signal_date,
#         "pm_state":      pm_state,
#         "entry_date":    entry_ts.strftime("%Y-%m-%d %H:%M ET"),
#         "entry_price":   round(entry_price, 4),
#         "exit_date":     exit_ts.strftime("%Y-%m-%d %H:%M ET"),
#         "exit_price":    round(exit_price, 4),
#         "exit_reason":   result["reason"],
#         "pnl_pct":       round(pnl_pct * 100, 3),
#         "pnl_dollar":    round(pnl_pct * POSITION_SIZE, 2),
#         "strategy":      "S1_10m_stop",
#     }
#
#
# # ─────────────────────────────────────────────────────────────────────────────
# #  Strategy 2 — full trade lifecycle
# # ─────────────────────────────────────────────────────────────────────────────
#
# def run_trade_s2(symbol: str, signal_date: str, direction: str) -> dict | None:
#     entry_date = next_trading_day(signal_date)
...
#         return None
#
#     # Align to nearest 1h bar at or before 10m entry
#     pre_1h = bars_1h[bars_1h.index <= entry_ts]
#     if pre_1h.empty:
#         return None
#     from_1h      = pre_1h.index[-1]
#     trail_active = False
#     trail_stop   = 0.0
#
#     # Walk 1h bars
...
#     while True:
#         result = check_exit_1h(bars_1h, from_1h, entry_price,
#                                direction, trail_active, trail_stop)
#         if result["status"] == "EXIT":
#             exit_ts    = result["exit_ts"]
#             exit_price = result["exit_price"]
#             exit_reason = result["reason"]
...
#
#         # Advance from_1h to last processed bar
#         remaining = bars_1h[bars_1h.index > from_1h]
#         if remaining.empty:
#             break
#         from_1h = remaining.index[-1]
#
#         # Check max hold
#         if from_1h.date() > entry_date + timedelta(days=MAX_HOLD_DAYS):
#             break
#
#     if exit_ts is None:
#         last_1h    = bars_1h.iloc[-1]
#         exit_ts    = bars_1h.index[-1]
#         exit_price = float(last_1h["close"])
...
#     print(f"  Worst trade  : {worst['symbol']} {worst['pnl_pct']:+.2f}%  (${worst['pnl_dollar']:+.2f})")
#     print(f"  Total P&L    : ${total_pnl:+,.2f}")
#     print(f"  Portfolio    : ${final_port:,.2f}  ({port_ret:+.2f}% return on $100K)")
#
#     # Exit reason breakdown
#     print(f"\n  Exit reasons:")
#     for reason, grp in df.groupby("exit_reason"):
#         avg_r = grp["pnl_pct"].mean()
#         print(f"    {reason:<16}  {len(grp):>4} trade(s)  avg {avg_r:+.2f}%")
#
#     # Pre-market state breakdown
#     if "pm_state" in df.columns:
#         print(f"\n  Pre-market EMA state vs outcome:")
#         for state, grp in df.groupby("pm_state"):
#             avg_r = grp["pnl_pct"].mean()
#             wr    = (grp["pnl_dollar"] > 0).mean() * 100
#             print(f"    {state:<10}  {len(grp):>4} trade(s)  win rate {wr:.0f}%  avg {avg_r:+.2f}%")
#
#     # Trade ledger
#     print(f"\n  {'#':<4} {'Sym':<7} {'Dir':<8} {'Scan':<5} {'PM':<8} "
#           f"{'Entry Date & Time':<22} {'Entry$':>9} "
#           f"{'Exit Date & Time':<22} {'Exit$':>9} "
#           f"{'%P&L':>7} {'$P&L':>8} {'Cum$P&L':>10}  Reason")
#     print(f"  {'─'*148}")
#
...
#         scan  = r.get("scanner", "–") or "–"
#         pm    = r.get("pm_state", "–") or "–"
#         print(f"  {int(r['trade_#']):<4} {r['symbol']:<7} {r['direction']:<8} "
#               f"{scan:<5} {pm:<8} "
#               f"{str(r['entry_date']):<22} ${r['entry_price']:>8.3f} "
#               f"{str(r['exit_date']):<22} ${r['exit_price']:>8.3f} "
#               f"{r['pnl_pct']:>+6.2f}% "
#               f"{arrow}${abs(r['pnl_dollar']):>7.2f} "
#               f"${r['cum_pnl']:>+9.2f}  {r['exit_reason']}")
#
...
#
# def main():
#     parser = argparse.ArgumentParser(description="EMA Backtest — Alpaca API")
#     parser.add_argument("--start-date",   default=None,
#                         help="Start date YYYY-MM-DD (required unless --signals-csv)")
#     parser.add_argument("--signals-csv",  default=None,
#                         help="Pre-saved signals CSV (symbol, signal_date, direction, scanner)")
#     parser.add_argument("--symbol",       default=None,
#                         help="Single symbol mode (skips full watchlist)")
#     parser.add_argument("--scanner",      type=int, choices=[1, 2], default=None,
#                         help="Daily scanner to use: 1=5/12 cross, 2=5/12×34/50 (default: both)")
#     parser.add_argument("--strategy",     choices=["1", "2", "both"], default="both",
#                         help="Backtest strategy: 1=10m stop, 2=1h stop+trail (default: both)")
#     parser.add_argument("--no-bearish",   action="store_true",
#                         help="Skip bearish signals (long-only backtest)")
#     parser.add_argument("--out",          default=None,
#                         help="Output CSV filename prefix (default: bt_alpaca_YYYYMMDD)")
#     args = parser.parse_args()
#
#     if not _API_KEY or not _SECRET_KEY:
#         print("\n[ERROR] ALPACA_API_KEY / ALPACA_SECRET_KEY not found in .env\n")
...
#         parser.error("Provide --start-date or --signals-csv")
#
#     # ── Step 1: Load or generate signals ──────────────────────────────────────
#     if args.signals_csv:
#         print(f"\n  Loading signals from {args.signals_csv} …")
#         sig_df = pd.read_csv(args.signals_csv)
#     else:
...
#     if args.no_bearish:
#         sig_df = sig_df[sig_df["direction"] == "BULLISH"]
#         print(f"  [--no-bearish] Filtered to {len(sig_df)} bullish signal(s).")
#
#     # ── Step 2: Run backtests ─────────────────────────────────────────────────
#     s1_trades, s2_trades = [], []
#
#     print(f"\n{'='*70}")
#     print(f"  STEP 2 — BACKTEST  ({len(sig_df)} signal(s))")
#     print(f"{'='*70}")
#
#     for _, row in sig_df.iterrows():
#         sym    = str(row["symbol"])
...
#                 t["scanner"] = scnr
#                 s1_trades.append(t)
#                 parts.append(f"S1: {t['pnl_pct']:+.2f}%  pm={t['pm_state']}")
#             else:
#                 parts.append("S1: no entry")
#
#         if args.strategy in ("2", "both"):
...
#                 t["scanner"] = scnr
#                 s2_trades.append(t)
#                 parts.append(f"S2: {t['pnl_pct']:+.2f}%  trail={t.get('trail_used', False)}")
#             else:
#                 parts.append("S2: no entry")
#
#         print("  |  ".join(parts))
...
#     if s1_trades:
#         df1 = make_summary(s1_trades)
#         print_report(df1, "STRATEGY 1 — 10m EMA STOP LOSS")
#         f1 = f"{out_prefix}_s1.csv"
#         df1.to_csv(f1, index=False)
#         print(f"  📄 Strategy 1 results → {f1}")
#
#     if s2_trades:
#         df2 = make_summary(s2_trades)
#         print_report(df2, "STRATEGY 2 — 1h EMA STOP + 3% TRAILING")
#         f2 = f"{out_prefix}_s2.csv"
#         df2.to_csv(f2, index=False)
#         print(f"  📄 Strategy 2 results → {f2}\n")
#
#     if not s1_trades and not s2_trades:
#         print("\n  No trades completed. Check symbol availability or date range.\n")
#
#
# if __name__ == "__main__":
#     main()