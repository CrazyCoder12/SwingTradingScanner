...
Data sources:
  ...
  • 1-min RTH bars       → 9:30–9:35 open-window performance (auto, when available)
  • Sector ETF snapshot  → confirms sector move is broad, not just 1-2 stocks

RTH bar availability (auto-detected — no flag needed):
  ┌─────────────────────┬──────────────────────────────────────────────────┐
  │ Run time (EST)      │ Mode                                             │
  ├─────────────────────┼──────────────────────────────────────────────────┤
  │ Before 9:30 AM      │ PM-ONLY — build watchlist before open            │
  │ After  9:50 AM      │ FULL    — RTH open bars included (IEX 15m delay) │
  │ After  9:35 AM      │ FULL    — RTH open bars included (SIP real-time) │
  └─────────────────────┴──────────────────────────────────────────────────┘
  In PM-only mode, RTH columns (RTH Open/High/Close, Gain%) show as — .
  Everything else (gap, VWAP, RVOL, score, quality) works identically.

Delay note:
  Free IEX feed = 15-min delayed.
  • Best PM-only run time : 4:00 AM – 9:29 AM EST
  • Best full run time    : 9:50 AM+ EST
  • For real-time         : set ALPACA_DATA_FEED=sip (paid ~$9/mo)
...
Usage:
  python sector_scanner.py --date 2026-06-25          # auto-detects PM-only or full
  python sector_scanner.py --start 2026-06-16 --end 2026-06-20
  python sector_scanner.py --date 2026-06-25 --no-rvol
  python sector_scanner.py --date 2026-06-25 --no-etf
  python sector_scanner.py --date 2026-06-25 --debug --csv
...
# IEX free feed adds ~15 min delay; RTH bars won't appear until ~9:50 AM
# SIP (paid) is real-time; RTH bars available from 9:30 AM
RTH_AVAILABLE_HOUR_IEX = 9
RTH_AVAILABLE_MIN_IEX  = 50     # 9:50 AM for IEX (15-min delay + buffer)
RTH_AVAILABLE_HOUR_SIP = 9
RTH_AVAILABLE_MIN_SIP  = 35     # 9:35 AM for SIP (real-time)
...
def is_rth_available(scan_date: date) -> bool:
    """
    Auto-detect whether RTH open bars (9:30–9:35) are likely available yet.

    Logic:
      - If scan_date is in the past → always True (historical run)
      - If scan_date is today:
          IEX (free, 15-min delay) → available ~9:50 AM EST
          SIP (paid, real-time)    → available ~9:35 AM EST
    """
    today = datetime.now(EST).date()
    if scan_date < today:
        return True   # historical — data is always there

    now_est = datetime.now(EST)
    if DATA_FEED == "sip":
        cutoff_h, cutoff_m = RTH_AVAILABLE_HOUR_SIP, RTH_AVAILABLE_MIN_SIP
    else:
        cutoff_h, cutoff_m = RTH_AVAILABLE_HOUR_IEX, RTH_AVAILABLE_MIN_IEX

    cutoff = now_est.replace(hour=cutoff_h, minute=cutoff_m, second=0, microsecond=0)
    return now_est >= cutoff
...
def scan_day(scan_date: date) -> dict:
    ...
    # ── Determine mode ────────────────────────────────────────────
    rth_mode = is_rth_available(scan_date)
    mode_label = "FULL (PM + RTH open bars)" if rth_mode else "PM-ONLY (watchlist builder)"
    if not rth_mode:
        if DATA_FEED == "sip":
            eta = "9:35 AM EST"
        else:
            eta = "9:50 AM EST"
        mode_label += f"  →  RTH bars available after {eta}"

    log.info("=" * 64)
    log.info("Scanning %s  |  feed=%s  |  mode=%s", scan_date, DATA_FEED, mode_label)
    log.info("  top_sectors=%d  |  rvol=%s  |  etf_filter=%s",
             TOP_SECTORS,
    ...
    log.info("Step 1/%d  Fetching snapshots for %d symbols…",
             3 if rth_mode else 2, len(all_snap_syms))
    ...
    # ── Step 2: PM 1-min bars ─────────────────────────────────────
    pm_candidates = [s for s in ALL_TICKERS if snaps.get(s, {}).get("pm_volume", 0) > 0]
    log.info("Step 2/%d  Fetching PM 1-min bars for %d tickers…",
             3 if rth_mode else 2, len(pm_candidates))
    pm_bars = pd.DataFrame()
    if pm_candidates:
    ...
    # ── Step 3: RTH bars (only if available) ──────────────────────
    rth_bars = pd.DataFrame()
    if rth_mode:
        ...
        if rth_bars.empty:
            # Market genuinely closed (holiday/weekend) — not a timing issue
            log.warning("RTH bars came back empty — market may be closed on %s", scan_date)
            log.warning("Falling back to PM-only mode.")
            rth_mode = False
    else:
        log.info("Step 2/2  Skipping RTH bars (pre-open or delayed feed — PM-only mode)")
    ...
    for sym in ALL_TICKERS:
        ...
        if rth_mode:
            rth_sym = get_sym(rth_bars, sym)
            if rth_sym.empty:
                # RTH was expected but this ticker missing — skip only in full mode
                skip["no_rth"] += 1
                if DEBUG:
                    log.debug("SKIP %-8s  no RTH bars", sym)
                continue
            rth_open  = float(rth_sym["open"].iloc[0])
        ...
        results.append({
            ...
            "confluence":      score,
            "gap_quality":     quality,
            # RTH fields — None in PM-only mode, populated in full mode
            "rth_open":        round(rth_open, 2)    if rth_open    else None,
            "rth_high":        round(rth_high, 2)    if rth_high    else None,
            "rth_close":       round(rth_close, 2)   if rth_close   else None,
            "open_gain_pct":   round(open_gain * 100, 2) if open_gain is not None else None,
        })
    ...
    etf_sector_stocks = build_etf_sector_list(snaps, etf_gaps, pm_bars, rth_bars, rth_mode)
...
def print_results(res: dict):
    ...
    rth_mode = res.get("rth_mode", False)

    # Column sets differ by mode
    if rth_mode:
        STOCK_COLS = [
    ...
    mode_banner = (
        "📋 PM-ONLY MODE — Watchlist for pre-open prep  (RTH columns omitted)"
        if not rth_mode else
        "📊 FULL MODE — PM + RTH open-window data"
    )
    ...
    if not rth_mode:
        feed_eta = "9:35 AM EST (SIP)" if DATA_FEED == "sip" else "9:50 AM EST (IEX)"
        print(f"  TIP    : Re-run after {feed_eta} for RTH open bars + Gain%")
    print(f"{'═'*80}\n")
    ...
    if etf_stocks:
        ...
        if not rth_mode:
            print(f"  NOTE  : RTH columns omitted (PM-only mode)")
        print(f"{'═'*80}")
