...
Usage
-----
    python -m swing_trading_scanner.scanners.confluence --start-date 2026-01-01
    python -m swing_trading_scanner.scanners.confluence --start-date 2026-01-01 --symbol NVDA
    python -m swing_trading_scanner.scanners.confluence --start-date 2026-01-01 --scanner 1
    python -m swing_trading_scanner.scanners.confluence --start-date 2026-01-01 --scanner 2
    python -m swing_trading_scanner.scanners.confluence --start-date 2026-01-01 --min-score 5      # high confluence only
    python -m swing_trading_scanner.scanners.confluence --start-date 2026-01-01 --min-atr 1.5      # only stocks moving >1.5%/day
    python -m swing_trading_scanner.scanners.confluence --start-date 2026-01-01 --no-atr-filter    # skip ATR viability gate
"""
...
def _build_row(
    ...
    row = {
        ...
        "bb_wpct": round(float(cur["bb_wpct"]), 1)
        if not pd.isna(cur["bb_wpct"])
        else "—",
        "atr_pct": round(float(cur["atr_pct"]), 2),
        "vol_ratio": round(float(cur["volume"]) / float(cur["vol_ma"]), 2)
        if not pd.isna(cur["vol_ma"])
        else "—",
    ...
    for i in range(start_pos, len(df)):
        ...
        if bull_cross and close > e9:
            ...
            if _score(mk, rk, sk, sq, vk) >= min_score:
                row = _build_row(
                    "BULLISH", date, cur, prev, e9, e21, mk, rk, sk, sq, vk
                )
                row["symbol"] = None
                signals.append(row)
            ...
            if _score(mk, rk, sk, sq, vk) >= min_score:
                row = _build_row(
                    "BEARISH", date, cur, prev, e9, e21, mk, rk, sk, sq, vk
                )
                row["symbol"] = None
                signals.append(row)
...
def _table(rows, direction, scanner):
    ...
    hdr = (
        f"  {'Symbol':<8}  {'Date':<12}  {'Score':<12}  {'Close':>9}  "
        f"{'EMA9':>9}  {'EMA21':>9}  "
...
def _print_legend(min_atr_pct, atr_filter):
    ...
    print(f"{'─' * _W}")
    print(
        f"  EMA {EMA_FAST}/{EMA_SLOW}      Primary crossover trigger (S1) / fast cluster (S2)"
    )
    print(f"  EMA {EMA_MID}/{EMA_TREND}    Major trend filter (S2 only)")
    print(
        f"  MACD ({MACD_FAST}/{MACD_SLOW}/{MACD_SIGNAL})  Line vs Signal + histogram direction  (+1)"
    )
    print(
        f"  RSI ({RSI_PERIOD})       Bull zone {RSI_BULL_LOW}–{RSI_BULL_HIGH}  |  Bear zone {RSI_BEAR_LOW}–{RSI_BEAR_HIGH}  (+1)"
    )
    print(f"  Stoch ({STOCH_K})    Slow %K/%D crossover in non-extreme zone  (+1)")
    print(
        f"  BB Width%    Squeeze = bottom {BB_SQUEEZE_PCT}% of 1yr range (volatility coil)  (+1)"
    )
    print(
        f"  VolRatio     Volume / {VOL_MA_PERIOD}-day avg — breakout confirmation  (+1)"
    )
    print(
        f"  ATR%         ATR({ATR_PERIOD}) / close — options viability (move size relative to price)"
    )
    if atr_filter:
        print(
            f"               Filter ON: signals require ATR% >= {min_atr_pct}%  (enough daily move to trade options)"
        )
    else:
...
def run_scanner(
    symbols, start_date_str, run_s1, run_s2, min_score, min_atr_pct, atr_filter
):
...
def run_single(
    symbol, start_date_str, run_s1, run_s2, min_score, min_atr_pct, atr_filter
):
    start_dt = datetime.strptime(start_date_str, "%Y-%m-%d")
    end_dt = datetime.now()
    today_str = end_dt.strftime("%Y-%m-%d")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--start-date", required=True, help="Scan start date  YYYY-MM-DD"
    )
    parser.add_argument(
        "--symbol", default=None, help="Single symbol (skips full watchlist)"
    )
    parser.add_argument(
        "--scanner",
        type=int,
