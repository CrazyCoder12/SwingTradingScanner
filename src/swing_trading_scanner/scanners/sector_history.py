"""
S&P 500 Premarket Sector Scanner - Historical Snapshot (Standalone)
==================================================================
Complete standalone script - fetches historical data for past dates

Features:
  - Works with PAST dates using Yahoo Finance free data
  - Captures 9:45 AM EST snapshot (using intraday 1-minute bars)
  - Saves results with timestamp
  - Same filters and scoring as original

Usage:
  # Get snapshot for a past date (uses historical 1-min data)
  python -m swing_trading_scanner.scanners.sector_history --date 2026-06-24 --snapshot

  # Get snapshot for today (waits for 9:45 AM)
  python -m swing_trading_scanner.scanners.sector_history --date 2026-06-25 --snapshot

  # Relax filters
  python -m swing_trading_scanner.scanners.sector_history --date 2026-06-24 --snapshot --no-rvol --no-etf
"""
...
def fetch_live_data(symbols: List[str]) -> Dict[str, Dict]:
    """Fetch live premarket data from Yahoo Finance"""
...
def fetch_historical_snapshot(
    symbols: List[str], target_date: date, target_time: str = "09:45"
) -> Dict[str, Dict]:
    """
    Fetch historical data at exact time using 1-minute bars
    Yahoo Finance free tier provides intraday data
    """
    results: Dict[str, Dict] = {}
    ...
    target_dt = target_dt.replace(hour=t_hour, minute=t_min, second=0, microsecond=0)
    target_dt = EST.localize(target_dt)

    # Convert to UTC for Yahoo
    target_utc = target_dt.astimezone(UTC)

    # Also get previous day for prev_close
    ...
    log.info(
        f"📊 Fetching historical data for {len(symbols)} symbols at {target_time} EST..."
    )

    chunk_size = 30  # Smaller chunks for historical data
    success_count = 0
    ...
    for i in range(0, len(symbols), chunk_size):
        ...
        for sym in chunk:
            try:
                ...
                # Fetch 1-minute data for the day (Yahoo provides up to 7 days of 1-min data)
                # We need to fetch a few days to get intraday data
                ...
                if hist.empty:
                    log.debug(f"No historical data for {sym}")
                    continue

                # Find the bar closest to target time
                # Yahoo returns data in UTC, convert to EST for comparison
                hist_est = hist.copy()
                hist_est.index = hist_est.index.tz_convert(EST)

                # Filter to target date
                target_str = target_date.strftime("%Y-%m-%d")
                day_data = hist_est[hist_est.index.strftime("%Y-%m-%d") == target_str]

                if day_data.empty:
                    log.debug(f"No data for {sym} on {target_str}")
                    continue

                # Find the bar closest to target time
                target_td = timedelta(hours=t_hour, minutes=t_min)
                day_data["time_diff"] = abs(
                    day_data.index.time
                    - datetime.min.time().replace(hour=t_hour, minute=t_min)
                )
                ...
                prev_day = hist_est[
                    hist_est.index.strftime("%Y-%m-%d")
                    == prev_date.strftime("%Y-%m-%d")
                ]
                prev_close = (
                    float(prev_day["Close"].iloc[-1]) if not prev_day.empty else 0
                )

                results[sym] = {
                    "prev_close": prev_close,
                    "pm_open": float(row["Open"]),
                    "pm_high": float(row["High"]),
                    "pm_low": float(row["Low"]),
                    "pm_close": float(row["Close"]),
                    "pm_volume": float(row["Volume"]),
                }
                success_count += 1
        ...
        log.info(f"  Progress: {success_count}/{len(symbols)} symbols fetched")
        time.sleep(0.3)  # Rate limiting

    log.info(f"✅ Successfully fetched {success_count}/{len(symbols)} symbols")
    return results


def avg_daily_volume_20d(
    symbol: str, target_date: Optional[date] = None
) -> Optional[float]:
    """Calculate 20-day average daily volume up to target date"""
    try:
...
def vwap_from_bars(
    symbol: str, target_date: Optional[date] = None, target_time: str = "09:45"
) -> Optional[float]:
    """Approximate VWAP from historical bars"""
    try:
        ticker = yf.Ticker(symbol)
        ...
        # Calculate VWAP from intraday data up to target time
        t_hour, t_min = map(int, target_time.split(":"))
        target_dt = datetime.combine(target_date, datetime.min.time())
        target_dt = target_dt.replace(hour=t_hour, minute=t_min)

        # Filter data up to target time
        hist_est = hist.copy()
        hist_est.index = hist_est.index.tz_convert(EST)
        day_data = hist_est[hist_est.index <= pd.Timestamp(target_dt, tz=EST)]
        ...
        total_value = (typical_prices * day_data["Volume"]).sum()
        ...
        log.debug(f"VWAP error for {symbol}: {e}")
        return None
...
def build_etf_sector_list(
    ...
    for sector, etf_gap_pct in top_etf_sectors:
        ...
        for sym in tickers:
            ...
            # For historical data, use stored vwap or calculate
            if "pm_vwap" in snap and snap["pm_vwap"]:
                pm_vwap_val = snap["pm_vwap"]
            else:
                pm_vwap_val = None
...
def wait_for_snapshot_time(target_time: str = "09:45"):
    ...
    if now >= target:
        log.info(
            f"⏰ Current time {now.strftime('%H:%M')} is past snapshot time {target_time}"
        )
        return
...
def scan_day_snapshot(
    scan_date: date, snapshot_time: str = "09:45", wait: bool = True
) -> Dict:
    ...
    is_past = scan_date < today

    if is_past:
        log.info(f"📅 Fetching HISTORICAL data for {scan_date} at {snapshot_time} EST")
        # Use historical data for past dates
        snaps = fetch_historical_snapshot(all_snap_syms, scan_date, snapshot_time)

        # Calculate VWAP for historical data
        log.info("📊 Calculating VWAP for historical data...")
        for sym in list(snaps.keys()):
            vwap_val = vwap_from_bars(sym, scan_date, snapshot_time)
            if vwap_val:
                snaps[sym]["pm_vwap"] = vwap_val

        # Use the snapshot time as the recorded time
        snapshot_dt = datetime.combine(scan_date, datetime.min.time())
        t_h, t_m = map(int, snapshot_time.split(":"))
        snapshot_dt = snapshot_dt.replace(hour=t_h, minute=t_m, second=0, microsecond=0)
        snapshot_dt = EST.localize(snapshot_dt)

    else:
        # For today, wait for the snapshot time if needed
        if wait:
            wait_for_snapshot_time(snapshot_time)

        snapshot_dt = datetime.now(EST)
        log.info(
            f"📸 Capturing LIVE snapshot at: {snapshot_dt.strftime('%H:%M:%S')} EST"
        )
        snaps = fetch_live_data(all_snap_syms)

        # Calculate VWAP for live data
        log.info("📊 Calculating VWAP for live data...")
        for sym in list(snaps.keys()):
            vwap_val = vwap_from_bars(sym)
            if vwap_val:
                snaps[sym]["pm_vwap"] = vwap_val
    ...
    for sym in ALL_TICKERS:
        ...
        if USE_RVOL:
            ...
            if avg_vol and avg_vol > 0:
                # Premarket volume is already total, compare to avg daily
                rvol = pm_vol / (
                    avg_vol * 0.20
                )  # 20% of daily volume expected in premarket
    ...
    log.info(
        "  Results → pass=%d | no_snap=%d gap=%d pmvol=%d vwap=%d rvol=%d etf=%d",
        len(results),
        skip["no_snap"],
        skip["gap"],
        skip["pmvol"],
...
def print_results(res: Dict):
    ...
    print(f"\n{'═' * 80}")
    print(
        f"  📸 SNAPSHOT SCANNER - {'HISTORICAL' if res.get('is_historical') else 'LIVE'}"
    )
    ...
    if etf_stocks:
        ...
        print(
            f"  Top {TOP_ETF_SECTORS} ETF sectors | Relaxed: gap≥{ETF_GAP_MIN * 100:.1f}%, vol≥{ETF_VOL_MIN:,}"
        )
        print(f"{'═' * 80}")
...
def save_snapshot(res: Dict, out: str = "."):
    ...
    # Create date folder
    date_folder = os.path.join(out, d)
    os.makedirs(date_folder, exist_ok=True)

    # Save all passing stocks
    fp = os.path.join(date_folder, f"snapshot_{timestamp}_all.csv")
    ...
    for sector, df in res["top_sectors"].items():
        p = os.path.join(
            date_folder, f"snapshot_{timestamp}_{sector.replace(' ', '_')}.csv"
        )
    ...
    summary = {
        ...
        "strong_count": len(
            res["all_passing"][
                res["all_passing"]["gap_quality"].str.startswith("STRONG")
            ]
        ),
        "moderate_count": len(
            res["all_passing"][
        ...
        "is_historical": res.get("is_historical", False),
    }
    jp = os.path.join(date_folder, f"snapshot_{timestamp}_summary.json")
    with open(jp, "w") as f:
...
def parse_args():
    p = argparse.ArgumentParser(
        description="S&P 500 Premarket Scanner - Historical Snapshots"
    )
    p.add_argument(
        "--date",
        metavar="YYYY-MM-DD",
        required=True,
        help="Date to scan (can be past or today)",
    )
    p.add_argument(
        "--snapshot",
        action="store_true",
        help="Capture snapshot at 9:45 AM (required for past dates)",
    )
    p.add_argument("--time", default="09:45", help="Snapshot time (HH:MM)")
    p.add_argument(
        "--no-wait",
        action="store_true",
        help="Don't wait for snapshot time (for today only)",
    )
    ...
    p.add_argument(
        "--bulk",
        action="store_true",
        help="Scan last 5 trading days (overrides --date)",
    )
    p.set_defaults(use_rvol=True, use_etf=True)
    return p.parse_args()


def get_previous_trading_days(n: int = 5) -> List[date]:
    """Get last n trading days"""
    dates = []
    current = datetime.now(EST).date()
    # If current time is before 9:45 AM, use yesterday as current
    now = datetime.now(EST)
...
def main():
    ...
    # Bulk mode - scan last 5 trading days
    if args.bulk:
        dates = get_previous_trading_days(5)
        log.info(f"📊 BULK MODE: Scanning {len(dates)} trading days")

        for scan_date in dates:
            log.info(f"\n{'=' * 70}")
            log.info(f"📅 Processing {scan_date}")

            if scan_date < datetime.now(EST).date():
                # Past date - use historical
                res = scan_day_snapshot(scan_date, args.time, False)
            else:
                # Today - wait if needed
            ...
            if res:
                ...
                print(f"❌ No results for {scan_date}")

            time.sleep(1)  # Rate limit between days

        print(f"\n✅ Complete! Check the {datetime.now(EST).date()} folder for results")
        return
    ...
    # Check if snapshot is required for past dates
    if scan_date < today and not args.snapshot:
        log.warning("⚠️  For past dates, you MUST use --snapshot flag")
        log.info(
            f"   Running: python -m swing_trading_scanner.scanners.sector_history --date {args.date} --snapshot"
        )
        return

    # Run scan
    if scan_date < today:
        # Past date - always use historical, no wait
        res = scan_day_snapshot(scan_date, args.time, False)
    else:
        # Today - wait if needed
        res = scan_day_snapshot(scan_date, args.time, not args.no_wait)
    ...
    if res:
        ...
        print("   --bulk              (scan last 5 trading days)")
