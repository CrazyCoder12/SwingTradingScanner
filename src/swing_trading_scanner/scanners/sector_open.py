...
Opening-window timing:
  This variant uses configured cutoffs of 09:50 for IEX and 09:35 for SIP.
  These are strategy assumptions, not provider latency guarantees. Coverage
  and access depend on your account. Missing bars may produce incomplete output.
...
Usage:
  python -m swing_trading_scanner.scanners.sector_open --date 2026-06-25          # auto-detects PM-only or full
  python -m swing_trading_scanner.scanners.sector_open --start 2026-06-16 --end 2026-06-20
  python -m swing_trading_scanner.scanners.sector_open --date 2026-06-25 --no-rvol
  python -m swing_trading_scanner.scanners.sector_open --date 2026-06-25 --no-etf
  python -m swing_trading_scanner.scanners.sector_open --date 2026-06-25 --debug --csv
...
# Strategy-specific availability cutoffs; not provider latency guarantees.
# SIP (paid) is real-time; RTH bars available from 9:30 AM
RTH_AVAILABLE_HOUR_IEX = 9
RTH_AVAILABLE_MIN_IEX = 50  # Configured IEX cutoff
...
def is_rth_available(scan_date: date) -> bool:
    ...
    Logic:
      ...
      - If scan_date is today:
          IEX → configured cutoff of 09:50 Eastern
          SIP → configured cutoff of 09:35 Eastern
    """
    today = datetime.now(EST).date()
    if scan_date < today:
        return True  # historical request — availability must still be checked
...
def scan_day(scan_date: date) -> dict:
    ...
    mode_label = (
        "FULL (PM + RTH open bars)" if rth_mode else "PM-ONLY (watchlist builder)"
    )
    if not rth_mode:
        if DATA_FEED == "sip":
            eta = "9:35 AM EST"
    ...
    log.info(
        "  top_sectors=%d  |  rvol=%s  |  etf_filter=%s",
        TOP_SECTORS,
    ...
    log.info(
        "Step 1/%d  Fetching snapshots for %d symbols…",
        3 if rth_mode else 2,
        len(all_snap_syms),
    )
    ...
    log.info(
        "Step 2/%d  Fetching PM 1-min bars for %d tickers…",
        3 if rth_mode else 2,
        len(pm_candidates),
    )
    pm_bars = pd.DataFrame()
    ...
    if rth_mode:
        ...
        if rth_bars.empty:
            ...
            log.warning(
                "RTH bars came back empty — market may be closed on %s", scan_date
            )
            log.warning("Falling back to PM-only mode.")
            rth_mode = False
    else:
        log.info(
            "Step 2/2  Skipping RTH bars (pre-open or delayed feed — PM-only mode)"
        )
    ...
    etf_sector_stocks = build_etf_sector_list(
        snaps, etf_gaps, pm_bars, rth_bars, rth_mode
    )
...
def print_results(res: dict):
    ...
    mode_banner = (
        ...
        if not rth_mode
        else "📊 FULL MODE — PM + RTH open-window data"
    )
