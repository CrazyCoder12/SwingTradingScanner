...
==================================================================
Yahoo Finance snapshot scanner; install dependencies through pyproject.toml.
...
Usage:
  ...
  python -m swing_trading_scanner.scanners.sector_snapshot --date 2026-06-25 --snapshot

  # Run immediately (no wait)
  python -m swing_trading_scanner.scanners.sector_snapshot --date 2026-06-25 --snapshot --no-wait

  # Run with custom time
  python -m swing_trading_scanner.scanners.sector_snapshot --date 2026-06-25 --snapshot --time 09:30

  # Relax filters
  python -m swing_trading_scanner.scanners.sector_snapshot --date 2026-06-25 --snapshot --no-rvol --no-etf
"""
...
def save_snapshot(res: Dict, out: str = "."):
    ...
    for sector, df in res["top_sectors"].items():
        p = os.path.join(
            out, f"{d}_snapshot_{timestamp}_{sector.replace(' ', '_')}.csv"
        )
...
def parse_args():
    p = argparse.ArgumentParser(
        description="S&P 500 Premarket Scanner - Time-Locked Snapshot"
    )
    p.add_argument("--date", metavar="YYYY-MM-DD", required=True)
    p.add_argument(
        "--snapshot", action="store_true", help="Capture snapshot at 9:45 AM"
    )
    p.add_argument("--time", default="09:45", help="Snapshot time (HH:MM)")
    p.add_argument(
        "--no-wait", action="store_true", help="Don't wait for snapshot time"
    )
