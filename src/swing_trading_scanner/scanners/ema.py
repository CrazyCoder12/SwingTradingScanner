...
Usage:
    python -m swing_trading_scanner.scanners.ema --start-date 2026-03-01
    python -m swing_trading_scanner.scanners.ema --start-date 2026-03-01 --symbol NVDA
    python -m swing_trading_scanner.scanners.ema --start-date 2026-01-01 --scanner 1
    python -m swing_trading_scanner.scanners.ema --start-date 2026-01-01 --scanner 2
"""
...
def _table_s2(rows: list, direction: str):
    ...
    print(
        f"\n{emoji} SCANNER 2 — {direction}  (EMA-12 crossed {label} EMA-34  +  {conf})"
    )
    if not rows:
        print("  No signals.")
        return
...
def run_scanner(symbols: list, start_date_str: str, run_s1: bool, run_s2: bool):
    ...
    s1_bull.sort(key=key)
    s1_bear.sort(key=key)
    s2_bull.sort(key=key)
    s2_bear.sort(key=key)
