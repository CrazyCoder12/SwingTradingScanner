...
Usage:
    Single day:   python -m swing_trading_scanner.backtests.ema --date 2024-01-15
    Date range:   python -m swing_trading_scanner.backtests.ema --start 2024-01-01 --end 2024-01-31
    With API keys: python -m swing_trading_scanner.backtests.ema --date 2024-01-15 --api-key YOUR_KEY --secret-key YOUR_SECRET
"""
...
load_environment()  # reads your .env file automatically
...
def fetch_alpaca_data(
    ...
    try:
        ...
        start_str = (
            f"{start.isoformat()}T06:00:00-04:00"  # include pre-market (6 AM ET)
        )
...
def backtest_stock_day(symbol: str, day_df: pd.DataFrame) -> list:
    ...
    for i in range(1, len(day_df)):
        ...
        trend_bullish = (
            (ema5 > ema34) and (ema12 > ema34) and (ema5 > ema50) and (ema12 > ema50)
        )
...
def run_backtest(
    start_date: date, end_date: date, api_key: str, secret_key: str
) -> pd.DataFrame:
    """
...
def display_results(trades_df: pd.DataFrame, start_date: date, end_date: date):
    ...
    stock_summary["Worst_Trade"] = (
        stock_summary["Worst_Trade"].round(2).astype(str) + "%"
    )
    ...
    if "entry_type" in trades_df.columns:
        print(f"\n\n📊 ENTRY TYPE BREAKDOWN\n")
        for etype, label in [
            ("EMA_CROSS", "EMA Cross (10-min charge)"),
            ("PM_BREAK_1", "Pre-Market Breakout (1st)"),
            ("PM_BREAK_2", "Pre-Market Breakout (2nd, after 3 PM)"),
        ]:
            ...
            print(
                f"  {label:35s}  Trades: {len(sub):4d}  Wins: {wins:4d} ({wins / len(sub) * 100:.1f}%)  Total P&L: {'+' if pnl >= 0 else ''}${pnl:.2f}"
            )
    ...
    print(
        tabulate(detail, headers=headers, tablefmt="rounded_outline", showindex=False)
    )
    ...
    avg_win = (
        trades_df[trades_df["return_dollar"] > 0]["return_dollar"].mean()
        if winning_trades > 0
        else 0
    )
    avg_loss = (
    ...
    print(
        tabulate(summary_data, headers=["Metric", "Value"], tablefmt="rounded_outline")
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python -m swing_trading_scanner.backtests.ema --date 2024-03-15 --api-key ABC --secret-key XYZ
  python -m swing_trading_scanner.backtests.ema --start 2024-01-01 --end 2024-01-31 --api-key ABC --secret-key XYZ
        """,
    )

    # Date arguments
    date_group = parser.add_mutually_exclusive_group(required=True)
    date_group.add_argument("--date", type=str, help="Single trading date (YYYY-MM-DD)")
    date_group.add_argument(
        "--start", type=str, help="Start date for range (YYYY-MM-DD) — use with --end"
    )
    parser.add_argument(
        "--end", type=str, help="End date for range (YYYY-MM-DD) — use with --start"
    )
    ...
    parser.add_argument("--secret-key", type=str, default="", help="Alpaca Secret Key")
