...
Usage:
    python -m swing_trading_scanner.backtests.swing --year 2025
    python -m swing_trading_scanner.backtests.swing --start 2025-01-01 --end 2025-12-31
"""
...
def fetch_daily_data(
    symbol: str, start: date, end: date, api_key: str, secret_key: str
    ...
    try:
        ...
        fetch_start = (
            datetime.combine(start, datetime.min.time()) - timedelta(days=WARMUP_DAYS)
        ).date()
...
def run_backtest(symbol_dfs: dict, start: date, end: date) -> tuple:
    ...
    all_days = sorted(
        {
            d
            for sym_df in symbol_data.values()
    ...
    for today in all_days:
        ...
        for sym in list(positions.keys()):
            ...
            if exit_reason:
                ...
                if position["half_exited"]:
                    total_pnl += (position["partial_px"] - entry) * position[
                        "partial_shares"
                    ]
                ...
                trades.append(
                    {
                        ...
                        "partial_px": round(position.get("partial_px") or 0, 2)
                        if position["half_exited"]
                        else None,
        ...
        for sym, pos in positions.items():
            if today in symbol_data[sym].index:
                pos_value += (
                    pos["shares_remaining"] * symbol_data[sym].loc[today]["close"]
                )
        ...
        deployed_pct = (
            (pos_value / portfolio_value_today * 100)
    ...
    for sym, position in list(positions.items()):
        ...
        return_pct = (total_pnl / invested * 100) if invested > 0 else 0
        hold_days = (
            date.fromisoformat(last_str) - date.fromisoformat(position["entry_date"])
        ).days
...
def compute_metrics(equity: pd.Series, initial: float, trading_days: int) -> dict:
...
def compute_buyhold_metrics(
    spy_df: pd.DataFrame, start: date, end: date, initial: float
) -> dict:
...
def display_results(
    ...
    print(
        f"  Capital : ${INITIAL_CAPITAL:,.0f}  |  Risk/trade: {RISK_PER_TRADE * 100:.0f}%  |  "
        f"Stop: {ATR_STOP_MULT}×ATR  |  Partial: {PARTIAL_RR:.0f}:1"
    )
    print(
        f"  Max pos.: {MAX_CONCURRENT} concurrent  |  Max/position: {MAX_POS_PCT * 100:.0f}%"
    )
    print(SEP)
    ...
    print(
        f"  {'Regime  (<200 EMA — bear mode)':<38} {filter_stats['n_regime_blocked']:>6}"
    )
    print(
        f"  {'Volatility (HV20 > ' + str(HV_PANIC) + '% panic)':<38} {filter_stats['n_hv_blocked']:>6}"
    )
    ...
    audit_rows = [
        ...
        [
            "Days with ≥1 open position",
            f"{days_in_market:>11} / {total_days} ({days_in_market / total_days * 100:.0f}%)",
        ],
    ]
    print(
        tabulate(
    ...
    monthly["Win%"] = (monthly["Wins"] / monthly["Trades"] * 100).round(1).astype(
        str
    ) + "%"
    ...
    comparison = [
        [
            "Starting Capital",
            f"${INITIAL_CAPITAL:>10,.0f}",
            f"${INITIAL_CAPITAL:>10,.0f}",
        ],
        ["Final Value", f"${final_val:>10,.0f}", f"${bh_final:>10,.0f}"],
        [
            "Total Return",
        ...
        ["Sharpe Ratio", f"{strat_m['sharpe']:>.2f}", f"{bh_m.get('sharpe', 0):>.2f}"],
    ...
    summary = [
        ...
        [
            ...
            f"${total_deployed:,.0f}  ({capital_turnover:.1f}x turnover)",
        ],
        ["Avg Daily Utilization", f"{avg_utilization:.1f}% of portfolio in market"],
        [
            "Days in Market",
            f"{days_in_market} / {total_days} trading days ({days_in_market / total_days * 100:.0f}%)",
        ],
    ...
    print(f"    python -m swing_trading_scanner.backtests.swing --year 2023")
    print(f"    python -m swing_trading_scanner.backtests.swing --year 2024")
    print(f"\n{SEP}\n")
...
Examples:
  python -m swing_trading_scanner.backtests.swing --year 2025
  python -m swing_trading_scanner.backtests.swing --start 2025-01-01 --end 2025-06-30
        """,
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--year", type=int, help="Full year (e.g. 2025)")
    group.add_argument(
        "--start", type=str, help="Start date YYYY-MM-DD (use with --end)"
    )
