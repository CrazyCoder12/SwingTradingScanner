...
Usage:
    python -m swing_trading_scanner.backtests.orb_ema --start 2026-01-01 --end 2026-03-26
"""
...
def backtest_day(symbol: str, day_df: pd.DataFrame) -> list:
    ...
    for candle_time, row in trade_df.iterrows():
        ...
        # ── PM EMA cross entry (15:00 – 15:49) ────────────
            ...
            if not any(
                pd.isna(v) for v in [prev_ema5, prev_ema12, cur_ema5, cur_ema12]
            ):
                close = row["close"]
                bullish_cross = (
                    prev_ema5 <= prev_ema12
                    and cur_ema5 > cur_ema12
                    and close > cur_ema12
                )
                bearish_cross = (
                    prev_ema5 >= prev_ema12
    ...
    if position is not None:
        ...
        direction = position["direction"]
        pnl_pct = (
            (close - entry) / entry if direction == "LONG" else (entry - close) / entry
        )
        trades.append(
            _make_trade(
                symbol,
...
def run_backtest(
    ...
    print(
        f"  Entry   : ORB breakout + VWAP + RVOL≥{RVOL_MIN}x + EMA {EMA_FAST}/{EMA_MID} vs {EMA_SLOW1}/{EMA_SLOW2}"
    )
    print(
        f"  ORB window : {ORB_END_TIME}–{ENTRY_CUTOFF}  |  PM cross: {PM_CROSS_START}–{FORCE_CLOSE_TIME}"
    )
...
def display_results(trades_df: pd.DataFrame, start_date: date, end_date: date):
    ...
    for direction in ["LONG", "SHORT"]:
        ...
        al = (
            sub[sub["return_dollar"] <= 0]["return_dollar"].mean()
            if (len(sub) - w) > 0
            else 0
        )
        print(
            f"  {direction:5s}  Trades:{len(sub):4d}  Wins:{w:3d}({w / len(sub) * 100:.1f}%)  "
    ...
    dd = (
        trades_df["return_dollar"].cumsum()
        - trades_df["return_dollar"].cumsum().cummax()
    ).min()
...
def parse_args():
    parser = argparse.ArgumentParser(
        ...
        epilog="Examples:\n"
        "  python -m swing_trading_scanner.backtests.orb_ema --date 2026-01-15\n"
        "  python -m swing_trading_scanner.backtests.orb_ema --start 2026-01-01 --end 2026-03-26",
    )
